...
# Try sqlalchemy - use lightweight fallback if not installed
try:
    ...
    # SQLAlchemy not installed – run in lightweight mode with in-memory store
    DB_AVAILABLE = False
    ...
    class FarmProfile:
        id = 1
        farmer_name = "Rithwik Rao"
        location_name = "Guntur, Andhra Pradesh"
        latitude = 16.3067
        longitude = 80.4365
        primary_crop = "Tomato & Potato"
        farm_size_acres = 4.5
        phone_number = "+91 8121985059"
    ...
    class _FakeSession:
        def add(self, obj):
            if isinstance(obj, ScanRecord):
                _scan_store.append(obj)
            elif isinstance(obj, VoiceCallLog):
                _call_store.append(obj)
    ...
    class _FakeQuery:
        ...
        def limit(self, n):
            ...
            return self
        def all(self):
            store = _scan_store if self._model is ScanRecord else _call_store
            n = getattr(self, '_n', len(store))
            return store[-n:]
